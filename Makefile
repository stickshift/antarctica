...
ANTARCTICA_PACKAGE_SRC := $(shell find src -type f | egrep -v '$(EXCLUDE_SRC)')
ANTARCTICA_PACKAGE_REQUIRES = $(ANTARCTICA_PACKAGE_SRC)
ANTARCTICA_PACKAGE := $(PACKAGES_DIR)/antarctica-$(PY_VERSION)-py3-none-any.whl

PACKAGES := $(PACKAGES) $(ANTARCTICA_PACKAGE)
...
$(ANTARCTICA_PACKAGE): $(ANTARCTICA_PACKAGE_REQUIRES) | $(PACKAGES_DIR) $(DEPENDENCIES)
	@echo
...
PHONIES := $(PHONIES) lint-nb lint-fmt lint-style
